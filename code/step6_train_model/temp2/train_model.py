...
class VideoSegDataset(Dataset):
    ...
    def __init__(self, split_dir: str, class_colors: List[Tuple[int,int,int]],
        ...
        self.class_colors = [tuple(c) for c in class_colors]  # all RGB triplets
    ...
    def _mask_to_onehot(self, mask_rgb: np.ndarray) -> np.ndarray:
        # mask_rgb: [H,W,3], returns [C,H,W] INCLUDING background
...
def build_class_colors(cfg: dict, split_dir: str, bg_rgb: Tuple[int,int,int]) -> List[Tuple[int,int,int]]:
    ...
    if cmap:
        ...
        return colors  # include background too
    else:
        raise RuntimeError("Please define full color_map in config (including background).")
...
def main():
    ...
    print(f"[INFO] Classes ({len(class_colors)} incl. background): {class_names}")
    ...
    if latest and os.path.isfile(latest):
        ...
        if state_dict["head.weight"].shape[0] != model_dict["head.weight"].shape[0]:
            print("[RESUME] Expanding head layer to include background class...")
            old_w, old_b = state_dict["head.weight"], state_dict["head.bias"]
            new_w, new_b = model_dict["head.weight"], model_dict["head.bias"]

            num_old = old_w.shape[0]
            new_w[:num_old] = old_w
            new_b[:num_old] = old_b

            state_dict["head.weight"] = new_w
            state_dict["head.bias"] = new_b

            # ⚠️ Reset optimizer state completely since shapes don’t match
            checkpoint["opt"] = opt.state_dict()  # fresh state dict
            print("[RESUME] Optimizer state reset due to head expansion.")
